...
def getOperator(packet)
  ...
  if packet[6] == '0'
    ...
    puts "packet[22...#{22+subLength}]: #{packet[22...22+subLength]}"
    processPacket(packet[22...(22+subLength)])
    ...
    #   si = s + (t*11)
    #   ei = si + 11
    #   puts "subPacket[#{si}..#{ei}]: #{packet[si..ei]}"
    #   processPacket(packet[si..ei])
    # end
    processPacket(packet[18..])
    pause('Op 1 - subPacket')
